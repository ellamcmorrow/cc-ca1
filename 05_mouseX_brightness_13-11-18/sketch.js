...
var numOfAgents;
  ...
  p.draw = function() {
    ...
    p.rect(0, 0, p.mouseX/50); //the overlay is a rectangle
    // Draw agents
    for (var i = 0; i < agentCount; i++) {
      if (drawMode == 1) {
        agents[i].update1(strokeWidth * mouseX, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(165,185),saturationValues[i] = p.random(80,100), brightnessValues[i] = (p.mouseX/10));
        p.fill(0,0,0); //overlay color
      ...
      //
        ...
        p.stroke(hueValues[i] = p.random(107,127), saturationValues[i] = p.random(4,69), brightnessValues[i] =(p.mouseX/10));

      }
      else if(drawMode==3){
        agents[i].update1(strokeWidth, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(55,60), saturationValues[i] = p.random(95,100), brightnessValues[i] = (p.mouseX/10)); //yellow

      }

      else if(drawMode==4){
        agents[i].update1(strokeWidth, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(0,20), saturationValues[i] = p.random(50,90), brightnessValues[i] = (p.mouseX/10));

      }
      else if(drawMode==5){
        agents[i].update1(strokeWidth, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(280,300), saturationValues[i] = p.random(80,100), brightnessValues[i] = (p.mouseX/10));
      }

      else if(drawMode==6){
        agents[i].update1(strokeWidth, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(255,275), saturationValues[i] = p.random(80,100), brightnessValues[i] = (p.mouseX/10));
      }
      else if(drawMode==7){
        agents[i].update1(strokeWidth, noiseScale, noiseStrength, noiseZVelocity);
        p.stroke(hueValues[i] = p.random(222,242), saturationValues[i] = p.random(48,68), brightnessValues[i] = (p.mouseX/10));
          }
        }
      }
